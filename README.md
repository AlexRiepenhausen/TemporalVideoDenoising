...
This phenomenon is the image noise we want to get rid off. Essentially, the goal here is to smooth out the RGB curve without
...
One possible choice for distinguishing between noise and real movement is the rolling variance taken over multiple frames (four in this case). Variance represents the expectation of the squared deviation from the mean and individual pixel values.
...
when computing the new average pixel value. If there is no movement, then up to 256 past frames can be used for producing the new, averaged frame. If the movement score is one, the number of frames to be used drops back to one immediately. In other words, if the number of frames is set to one then the current pixel will not be changed at all.
...
## The Results?
So much for the theory. Here are some actual side-by-side comaprisons between denoised and original video frames. The frames themselves were cropped to make the differences more pronounced:
...
<br/><br/>
The code was written in C++ and makes use of the OpenCV library. In terms of runtime, it takes roughly two minutes to denoise a Full HD video with a duration of ten seconds.
